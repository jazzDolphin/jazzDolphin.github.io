...
이전 포스트에서 통계량을 통해 data reduction을 수행할 때 $$\theta$$를 inference하는데 필요한 정보는 보존하면서도 불필요한 정보는 버리는 통계량을 선택하는 기준이 필요하다고 언급하였다.

이번 포스트에서는 그 첫번째 기준인 Sufficiency Principle에 대해 살펴보고자 한다.

## Sufficiency Principle
Parameter $$\theta$$에 대한 충분통계량(sufficient statistic)을 대략적으로 설명하면 샘플의 가진 $$\theta$$에 대한 정보를 모두 담고 있는 통계량이다. 

즉 충분통계량을 제외한 샘플의 다른 추가적인 정보는 $$\theta$$를 inference하는데 도움이 되지 않는다는 의미이다.
...
통계량 $$T(\mathbf{X})$$가 주어졌을 때의 샘플 $$\mathbf{X}$$의 조건부 분포가 $$\theta$$에 의존하지 않는다면, $$T(\mathbf{X})$$는 $$\theta$$에 대한 충분통계량이라고 한다.

$$T(\mathbf{X})$$가 이산 분포를 따른다고 가정하고 $$t$$가 $$T(\mathbf{X})$$의 image에 속하는 값이라고 하자. \\
(교재에서는 연속 분포를 따르는 경우는 Lehmann의 책(_Testing Statistical Hypotheses_
)을 참고하라고 언급한다. 아래 글을 읽다보면 연속 분포에서는 $$P_\theta(T(X)=t)$$가 항상 0이기에 $$P_\theta(\mathbf{X}=\mathbf{x} \mid T(\mathbf{X})=t)$$가 정의되지 않는다는 것을 알 수 있다.)
...
그러므로 $$T(\mathbf{x})=t$$인 경우, 즉 $$P_\theta(\mathbf{X}=\mathbf{x} \mid T(\mathbf{X})=T(\mathbf{x}))$$이 $$\theta$$에 값에 따라 변하는지 확인하면 된다.
...
위에서 언급한 것처럼 $$\mathbf{X}$$나 $$T(\mathbf{X})$$가 연속 분포를 따른다면 위의 조건부 확률을 이용한 수식 전개를 따를 수 없지만 $$\frac{p(\mathbf{x} \mid \theta)}{q(T(\mathbf{x}) \mid \theta)}$$를 이용한 충분통계량의 판별은 동일하게 할 수 있다고 한다.

결론적으로 $$p(\mathbf{x} \mid \theta)$$가 $$\mathbf{X}$$의 joint pdf or pmf이고, $$q(t \mid \theta)$$가 $$T(\mathbf{X})$$의 pdf or pmf라고 할 때, 모든 $$\mathbf{x} \in \mathcal{X}$$에 대해 $$\frac{p(\mathbf{x} \mid \theta)}{q(T(\mathbf{x}) \mid \theta)}$$가 $$\theta$$에 대한 상수함수라면 $$T(\mathbf{X})$$는 $$\theta$$에 대한 충분통계량이 된다.
...
$$X_1, \ldots, X_n \stackrel{i.i.d}{\sim} \mathcal{N}(\mu, \sigma^2)$$ ($$\sigma^2$$는 known)라 하자.
...
## Reference
- Casella, G., & Berger, R. L. (2002). Statistical inference (Vol. 2)
