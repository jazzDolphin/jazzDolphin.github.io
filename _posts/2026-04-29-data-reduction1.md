...
이러한 문제를 해결하고 샘플이 가진 정보를 잃지 않으면서 많은 샘플들의 정보를 요약하기 위해 통계량(Statistic)의 개념이 도입되었다.

(여기서 $$\mathbf{X} = [X_1, \dots, X_n]$$는 random variables, $$\mathbf{x} = [x_1, \dots x_n]$$는 관측된 표본을 의미한다.)
...
예를 들어 sample size가 $$n=3$$일 때 통계량 $$T(\mathbf{x})=x_1+x_2+x_3$$ 을 정의한다면, $$A_9$$는 표본의 합이 $$9$$가 되는 샘플들의 집합이다.
...
결과적으로 $$T(\mathbf{x})=9$$이라는 사실은 $$A_9$$에 속하는 수많은 원소 중 각 관측치가 구체적으로 어떤 값들을 가졌는지에 대한 raw data의 정보는 버리고, 오직 관측된 표본의 합이 9라는 요약된 정보만을 남긴다는 의미이다.

이것이 바로 통계량이 data reduction을 수행한다는 의미이다.

### Principle of Data Reduction

통계량을 이용해 data reduction을 수행할 때 임의의 통계량을  사용하게 된다면 parameter $\theta$를 inference하는데 필요한 정보까지 버려질 수 있다.

따라서 통계량을 이용한 data reduction을 수행할 때 $$\theta$$를 inference하는데 필요한 정보는 보존하면서도 불필요한 정보는 버리는 통계량을 선택하는 기준이 필요하다.
...
다음 포스트에서는 Sufficiency Principle에 대해 알아보도록 하자.
